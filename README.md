...
Пользовательский интерфейс документооборота СберНПФ на React, TypeScript и Vite. Работает с публичным API Corelia; поддерживает договоры ПДС, КИД ОПС, задачи, версии документов и вложения.
...
lint выполняет `tsc --noEmit`; build — `tsc && vite build`, результат в dist. Отдельного test-скрипта в package.json нет. Preview служит просмотру сборки. Для production-хостинга SPA требуется fallback на index.html для клиентских маршрутов.
